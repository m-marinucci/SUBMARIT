function [Output] = kSMNFold(SWM,NoClusters,MinItems,NoRuns,NFold,NoRandom,MaxFoldRun)
...
%AvAdjRandDist - Empirical distribution for the average adjusted Rand index
  ...
  for i=1:MaxFoldRun
    ...
    Temp=RunClusters(SubSWM,NoClusters,MinItems,NoRuns);
    %Assign the test items using a constrained version of the algorithm
    CFold.Clusters{i}=RunClustersConstrained(SWM,NoClusters,MinItems,Temp.Assign,TrainIndexes,TestIndexes,NoRuns);
    if NoRandom>0
      EmpDist.Random{i}=[];
